...
public class IronScaffoldingBlock extends Block implements Waterloggable {
   public static final int MAX_DISTANCE = 21;
   public static final IntProperty DISTANCE = IntProperty.of("distance", 0, 21);
   ...
   public IronScaffoldingBlock(AbstractBlock.Settings settings) {
      ...
      this.setDefaultState(
              (BlockState)((BlockState)((BlockState)((BlockState)this.stateManager.getDefaultState()).with(DISTANCE, 21)).with(WATERLOGGED, false))
                      .with(BOTTOM, false)
      );
   }
   ...
   public static int calculateDistance(BlockView world, BlockPos pos) {
      ...
      int i = 21;
      if (blockState.isOf(ContentBlocks.IRON_SCAFFOLDING)) {
      ...
      while (iterator.hasNext()) {
         ...
         if ((blockState2 = world.getBlockState(mutable.set(pos, iterator.next()))).isOf(ContentBlocks.IRON_SCAFFOLDING)
                 && (i = Math.min(i, (Integer)blockState2.get(DISTANCE) + 1)) == 1) {
            break;
         }
      }
   ...
   public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
      ...
      if ((Integer)blockState.get(DISTANCE) == 21) {
         if ((Integer)state.get(DISTANCE) == 21) {
            FallingBlockEntity.spawnFromBlock(world, pos, (BlockState)blockState.with(WATERLOGGED, false));
         } else {
   ...
   public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
      return calculateDistance(world, pos) < 21;
   }
